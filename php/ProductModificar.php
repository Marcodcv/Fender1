...
<body>  
  <div class="conten">
        ...
        <?php
          ...
          $SQL="UPDATE inventario SET  stock='$stock', modelo='$modelo', marca='$marca',tipo='$tipo', precio='$precio',Provedores_id='$provedor' WHERE  (id LIKE '$id')";
          if(mysqli_query($idcone,$SQL))
          {
            ?>
            <script>alert("Los Datos se han actualizado exitosamente.")</script>
            <?php
            header("Location: ProductListar.php");
          }else
          {
            ?>
