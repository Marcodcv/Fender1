...
<body>  
  <div class="conten">
        ...
        <?php
          ...
          $SQL="UPDATE clientes SET  nombre='$nombre', telefono='$telefono', correo='$correo',contrasena='$contraseña' WHERE  (id LIKE '$id')";
          if(mysqli_query($idcone,$SQL))
          {
            ?>
            <script>alert("Los Datos se han actualizado exitosamente.")</script>
            <?php
            header("Location: Clilistar.php");
          }else
          {
            ?>
            <script>alert("Los Datos no se han podido actualizar.")</script>
            <?php
            header("Location:../paginas/ClieNActualizar.html");
          }
          mysqli_close($idcone);
          ?>
        <br>
